"""
Tests unitarios para RejectUpdateGolfCourseUseCase.
"""
...
class TestRejectUpdateGolfCourseUseCase:
    """
    Suite de tests para RejectUpdateGolfCourseUseCase.

    Este use case permite al Admin rechazar un clone de actualización,
    eliminando el clone y marcando el original como sin cambios pendientes.
    """
    ...
    def original_and_clone(self):
        ...
        # Original APPROVED con pending update
    ...
    async def test_should_reject_update_and_delete_clone(self, mock_uow, original_and_clone):
        """
        Verifica que Admin puede rechazar clone y eliminarlo.

        Given: Clone válido con original_golf_course_id
        When: Admin rechaza el update
        Then: Clone eliminado, original sin cambios y sin marca pending
        """
        # Arrange
        ...
        request_dto = RejectUpdateGolfCourseRequestDTO(clone_id=str(clone.id))
        use_case = RejectUpdateGolfCourseUseCase(mock_uow)
        ...
        assert response.original_golf_course.name == "Original Golf Club"
        assert response.original_golf_course.country_code == "ES"
        assert response.original_golf_course.approval_status == ApprovalStatus.APPROVED.value
        assert response.original_golf_course.is_pending_update is False  # Marca quitada
        assert response.rejected_clone_id == str(clone.id)

        # Verificar llamadas
        mock_uow.golf_courses.save.assert_called_once()  # Original (solo clear_pending_update)
    ...
    async def test_should_raise_error_when_not_a_clone(self, mock_uow):
        ...
        request_dto = RejectUpdateGolfCourseRequestDTO(clone_id=str(normal_course.id))
        use_case = RejectUpdateGolfCourseUseCase(mock_uow)
