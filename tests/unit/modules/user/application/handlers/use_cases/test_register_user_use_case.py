...
from src.modules.user.application.dto.user_dto import RegisterUserRequest
...
class TestRegisterUserUseCase:
    ...
    async def test_should_register_user_successfully(self, uow: InMemoryUnitOfWork):
        ...
        async with uow:
            saved_user = await uow.users.find_by_id(user_response.id)
    ...
    async def test_should_raise_error_when_email_already_exists(
        ...
        use_case = RegisterUserUseCase(uow)
        existing_user_request = RegisterUserRequest(
        ...
        new_request_dto = RegisterUserRequest(
            email="existing.user@example.com",
